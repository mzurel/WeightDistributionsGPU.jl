...
## ---------------------------------- ##
## Encode GF(9) elements as 0x00:0x08 ##
## ---------------------------------- ##
...
"""
    ...
    Take a Nemo matrix of GF(9) elements and encode it as a UInt8 matrix.
"""
...
@inline function gfqadd(x::UInt8, y::UInt8, addtbl)::UInt8
    return @inbounds addtbl[Int(x)+1, Int(y)+1]
end

@inline function gfqmul(x::UInt8, y::UInt8, multbl)::UInt8
...
function partialweightskernel(Gt::CuDeviceMatrix{UInt8},
    ...
    while idx < final
        ...
        @inbounds for pos ∈ k-1:-1:0  # run unrank algorithm to fill vector with coefficients
            ...
            if comb ≥ c
                i = pos + 1
                coeff = UInt8(coeffs & 0x07) + 0x01
                for j ∈ 1:k
                    prod = gfqmul(coeff, Gt[j,i], multbl)
                    old = cw[j]
                    new = gfqadd(old, prod, addtbl)
                    cw[j] = new
                end
...
"""
    ...
    G is the generator matrix of a self-dual linear code over GF(9). This function returns
